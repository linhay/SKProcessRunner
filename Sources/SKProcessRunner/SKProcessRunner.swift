...
public extension SKProcessRunner {
    enum ShellMode: Sendable, Equatable {
        case login
    ...
    static func loadUserShellPATHSync(
        ...
        mode: ShellMode = .loginInteractive,
        timeoutMs: Int = 1_500
    ) -> String? {
        ...
        guard let result = try? runSync(executableURL: shellURL, arguments: args, configuration: config, throwOnNonZeroExit: false)
        else { return nil }
    ...
    static func resolveExecutableInUserShell(
        ...
        guard let result = try? await run(executableURL: shellURL, arguments: args, configuration: config, throwOnNonZeroExit: false)
        else { return nil }
